// A plain-JS copy of `src/data/products.ts`, for the standalone Node
// server in this folder. It exists separately because this server runs
// under plain Node (no TypeScript/Vite build step), so it can't import a
// `.ts` file directly. If you add/edit products, update both files — or
// better, if this server ever becomes more than a demo, replace this
// with a real database and drop the duplication entirely.
